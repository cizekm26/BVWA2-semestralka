...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
  ...
  if ($result && $result->num_rows > 0) {
    ...
    $message['obsah'] = decryptData($message['obsah']);
    $message['predmet'] = decryptData($message['predmet']);
...
<body class="bg-white">
  ...
  <?php 
    if(isset($message)){
      echo '<div class="px-4 sm:px-0 flex items-stretch justify-between flex-wrap">
      <h1 class="text-3xl font-extrabold">Zpráva</h1>
      <div>
        <a href="new-message.php?login='. $message['login'] .'" class="w-28 rounded-lg shadow-lg text-sm text-white bg-blue-500 px-2 py-3 uppercase font-semibold">Odpovědět</a>
    ...
    <div class="mt-5 bg-gray-100 shadow-md p-5 rounded divide-y divide-gray-600">
      <div class="flex items-stretch py-2 justify-between flex-wrap">
        <div>
          <span class="font-bold">Od: </span><span>'.$message["jmeno"] . " " . $message["prijmeni"].'</span>
        </div>
        <div>
          <span class="font-bold">Odesláno: </span><span>'.$message["cas_odeslani"].'</span>
        </div>
      </div>
